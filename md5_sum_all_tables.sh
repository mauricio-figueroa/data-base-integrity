...
#echo  $(mysql -h $MYSQL_HOST -u $MYSQL_USER -p$MYSQL_PASSWORD -D $MYSQL_DATABASE -e 'select *  from cash_provincias order by id_provincia desc limit 100 ' )   >> md5.txt



#sed -i '/cant/d' md5.txt
#sed -e "s/cant//g" -i md5.txt
...
echo "MD5:" $(md5sum md5.txt) >>md5_sum_tables_$MYSQL_HOST.txt
